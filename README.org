...
I often want to switch between a few common frame layouts. For example, a narrow centred frame when I want to focus on a single buffer. Or a wider frame to display two side by side buffers, aligned to the right of the screen, so I can display some other app on the left.
...
I choose to use [[https://github.com/akermu/emacs-libvterm/][vterm]] because it is fast, compatible with my usual terminal setup, and supports interactive commands.
