...
For major modes based on Tree-Sitter, also run ~M-x treesit-install-language-grammar~ for each relevant language.
...
When typing a key bind prefix, ~which-key-mode~ expands the minibuffer to display commands under that prefix.
